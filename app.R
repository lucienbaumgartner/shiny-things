...
server <- shinyServer(
  function(input, output, session) {
        ...
        text=
            lapply(1:nbins, function(x){
             ...
             'data %>% ggplot(., aes(x=1, y=y)) +
                      ...
                      scale_fill_discrete() +
                      theme_void()+
                      theme(plot.background=element_rect(fill=adjustcolor("lightblue", alpha.f = 0.2)), plot.margin = unit(c(0, 0, 0, 0), "null"))',
             '})'
             )
            })
      )
    ...
    observeEvent(input$submit, {
      saveData(formData())
      session$sendCustomMessage('userdata', paste0(formData(), collapse = ";"))
    })
  }
